...
def test_query_stream_returns_200_with_sse_content_type() -> None:
    ...
    try:
        response = _make_client().post("/query/stream", json={"question": "test?"})
        assert response.status_code == 200
...
def test_query_stream_body_contains_chunks_and_done_events() -> None:
    chunks_data = (
    ...
    app.dependency_overrides[get_pipeline] = lambda: pipeline
    try:
        response = _make_client().post("/query/stream", json={"question": "vacation?"})
...
def test_query_stream_calls_pipeline_with_question() -> None:
    pipeline = _mock_pipeline()
    pipeline.stream_query.return_value = iter(
        ["event: done\ndata: {}\n\n"]
    )
    ...
    try:
        _make_client().post(
            "/query/stream", json={"question": "What is the leave policy?"}
        )
...
def test_query_stream_empty_question_still_calls_pipeline() -> None:
    pipeline = _mock_pipeline()
    pipeline.stream_query.return_value = iter(
        ["event: done\ndata: {}\n\n"]
    )
    app.dependency_overrides[get_pipeline] = lambda: pipeline
    try:
        response = _make_client().post("/query/stream", json={"question": ""})
        assert response.status_code == 200
...
def test_query_returns_answer_and_chunks() -> None:
    ...
    app.dependency_overrides[get_pipeline] = lambda: pipeline
    try:
        response = _make_client().post("/query", json={"question": "VPN?"})
        assert response.status_code == 200
        data = response.json()
